...
export const PrimaryButton = ({
    ...
    return (
        <button
            ...
            onClick={onClick}
            disabled={disabled}
            className={`text-white font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 flex gap-2
        ${disabled
...
export const SecondaryButton = ({ children, onClick }: {
    ...
    return (
        <>
            <button type="button" onClick={onClick} className="text-white bg-sky-700 hover:bg-sky-800 cursor-pointer focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 flex gap-2"
            >{children}</button >
        </>
    )
}
